    ...
    return (
        <div className="bg-gradient-to-r from-gray-800 via-gray-900 to-gray-950 text-white rounded-2xl p-6 shadow-lg transition duration-300 transform hover:scale-105 hover:shadow-2xl border-b border-gray-500 " >
            ...
            <div className="mb-4">
                ...
                <p className="mb-2"><span className="font-semibold">Status:</span> <span className={`px-2 py-1 rounded-full text-sm ${appointment.status === 'approved' ? 'bg-green-500' : appointment.status === 'pending' ? 'bg-yellow-500' : 'bg-red-500'}`}>{appointment.status}</span></p>
